...
def main():
    ...
    parser.add_argument("--share-flashhead-weight", action="store_true")
    ...
    config = dict(model=args.model_id,
                  ...
                  share_flashhead_weight=args.share_flashhead_weight,
    ...
    sharing = None
    if args.share_flashhead_weight:
        from share_flashhead_weight import enable_shared_flashhead
        sharing = enable_shared_flashhead(args.model_dir)
    ...
    if sharing is not None:
        assert sharing.get("shared_storage"), "Shared FlashHead did not activate"
        (args.output / "weight-sharing.json").write_text(json.dumps(sharing, indent=2))
