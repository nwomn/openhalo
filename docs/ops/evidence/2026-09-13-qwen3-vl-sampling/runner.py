...
def read_frames(path, profile):
    ...
    if profile == "uniform12":
        indices = np.linspace(0, len(decoded) - 1, 12).round().astype(int).tolist()
    else:
...
def main():
    ...
    if args.prepare_only:
        ...
        for profile in PROFILES:
            ...
            canvas = Image.new("RGB", (4 * 384, 3 * 240), "white")
    ...
    import torch
    llm_args = dict(model="/work/assets/runtime-model", dtype="auto", max_model_len=6144,
                    max_num_seqs=1, max_num_batched_tokens=2048,
                    kv_cache_memory_bytes=768 * 1024 * 1024, gpu_memory_utilization=0.40,
    ...
    config = dict(model="cyankiwi/Qwen3-VL-2B-Instruct-AWQ-4bit",
                  ...
                  prompt=PROMPT, decoding=decoding, profiles=PROFILES, repeats=3,
    ...
    for repeat in range(3):
        runs.extend((p, repeat) for p in PROFILES[repeat:] + PROFILES[:repeat])
