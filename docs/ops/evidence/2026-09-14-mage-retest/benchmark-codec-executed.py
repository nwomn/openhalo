...
def prepare_codec(case, directory):
    # Use installed H.264/HEVC bit-cost decoder/selector, not pixel differencing.
    import contextlib
    import codec_selector.core.pipeline as pipeline
    from codec_video_prep.api import run_preinfer
    sys.path.insert(0,"/home/jetson/mage-vl-gguf/tools")
    from pack_mage_codec import pack
    path,expected,interval=CASES[case]
    digest=hashlib.sha256(Path(path).read_bytes()).hexdigest(); assert digest==expected
    cap=cv2.VideoCapture(path); n=int(cap.get(cv2.CAP_PROP_FRAME_COUNT)); cap.release()
    indices=np.rint(np.linspace(0,n-1,16)).astype(int).tolist()
    previous=pipeline.sample_frames
    # Pin only the sampler to the already verified comparison frame IDs.
    # Disable the package's keyframe-shift policy so it cannot change them.
    pipeline.sample_frames=lambda meta,cfg:(indices,dict(mode="explicit_matched_round_linspace",frame_ids=indices))
    cache=directory/(case+"-codec-cache"); dest=directory/(case+"-codec.mcv")
    try:
        with (directory/(case+"-codec-prep.log")).open("a") as log, contextlib.redirect_stdout(log):
            result=run_preinfer(video=path,out_dir=str(cache),num_sampled_frames=16,
                group_size=16,images_per_group=4,patch=16,max_pixels=399360,
                grouping_mode="fixed",avoid_keyframes=False,canvas_format="png",thread_count=1)
            pack(cache,dest)
    finally: pipeline.sample_frames=previous
    meta=json.loads((cache/"meta.json").read_text())
    positions=np.load(cache/"src_patch_position.npy",allow_pickle=False)
    valid=positions[positions[:,0]>=0]
    selected=sorted(np.unique(valid[:,0]).astype(int).tolist())
    assert set(selected).issubset(indices) and selected
    h,w=meta["processed_hw"]
    return [str(dest)],dict(input_sha256=digest,source_interval_s=interval,
        sampled_frame_indices=indices,sampled_times_s=[i/4 for i in indices],
        selected_patch_frame_indices=selected,selected_patch_times_s=[i/4 for i in selected],
        effective_frame_size=[h,w],visual_tokens=len(valid)//4,
        codec_summary=meta,geometry_adapter="codec-video-prep 0.2.5 H.264/HEVC bit-cost four-canvas selection; explicit same 16 sampled frames, package geometry and sparse patches")

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--root",type=Path,required=True)
    ap.add_argument("--cases",nargs="+",default=list(CASES)); ap.add_argument("--repeats",type=int,default=3)
    ap.add_argument("--profile",choices=["full","codec"],default="full")
    ...
    (args.root/"config.json").write_text(json.dumps(dict(command=command,prompt=PROMPT,max_tokens=96,
        cases=args.cases,repeats=args.repeats,profile=args.profile,decoding="greedy",n_ctx=8192,n_batch=1024,n_ubatch=256,
            ...
            with (args.root/"raw.jsonl").open("w") as out:
                for case in args.cases:
                    for repeat in range(-1,args.repeats):
                        ...
                        paths,metadata=(prepare if args.profile=="full" else prepare_codec)(case,args.root); prep_s=time.perf_counter()-start
                        proc.stdin.write("\t".join(paths)+"\n"); proc.stdin.flush()
                        actual=get(); assert actual["event"]=="input"
                        assert actual["frames"]==metadata.get("selected_patch_frame_indices",metadata["sampled_frame_indices"]),actual
                        assert actual["seconds"]==metadata.get("selected_patch_times_s",metadata["sampled_times_s"]),actual
