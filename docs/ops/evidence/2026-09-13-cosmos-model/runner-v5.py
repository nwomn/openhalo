...
from benchmark import MODEL, PROMPT
...
def main():
    ...
    llm_args = dict(
        model=str(args.root / "assets/model"), dtype="auto",
        ...
        limit_mm_per_prompt={"video": {"count": 1, "num_frames": 12,
                                      "width": args.input_width, "height": 480}, "image": 0, "audio": 0},
    ...
    config = dict(model=MODEL, prompt=PROMPT, llm_args=llm_args, max_tokens=args.max_tokens,
                  temperature=0, repeats=args.repeats, case_ids=[c["id"] for c in cases],
                  ...
                  multiprocessing=False, input_width=args.input_width,
    ...
    with (args.output / "raw.jsonl").open("w") as log:
        for case, repeat in runs:
            ...
            try:
                ...
                height = round(frames.shape[1] * args.input_width / frames.shape[2])
                frames = np.stack([cv2.resize(frame, (args.input_width, height)) for frame in frames])
                ...
                row.update(seconds=elapsed, text=output.text,
                           ...
                           actual_timestamps=re.findall(r"<[0-9.]+ seconds>", actual_prompt))
