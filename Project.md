...
- The owner-run `v0.1.0 -> v0.1.2` update completed and preserved Runtime state, but exposed a staging defect: candidate venv console scripts embedded their deleted `.staging-*` interpreter path, causing global `openhalo` and `openhalo-edge` commands to fail after activation. The installed v0.1.2 entrypoints were restored by reinstalling the verified v0.1.2 archive into its current venv without changing dependencies or personal data. Stable [`v0.1.3`](https://github.com/nwomn/openhalo/releases/tag/v0.1.3) is published from exact commit `af6ff4202352b79a7249285649e1f5d252ed97cc` with archive SHA-256 `80792a1100c30145dda472ae0b71501f03cde85aaf5227ae7d8ea006d1c664e6`; it atomically places verified source into the final release directory before creating the venv, removes that unselected candidate on installation failure, and therefore writes console-script shebangs against the stable commit path. Focused tests and an independent real venv/pip candidate staging exercise both executed the generated console script successfully from its final path. The owner must still complete one normal `v0.1.2 -> v0.1.3` update to accept the live activation path.
