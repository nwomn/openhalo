...
- 4.21. Define milestone M20: Agent Harness and runtime action-loop architecture refactor with unified action/tool governance, so the already-running OpenHalo system can replace its current agent-execution implementation with a Hermes-backed harness core behind an OpenHalo adapter, while preserving the nested internal agent loop, external runtime action loop, memory-consolidation model, LLMOps/eval gate, and one inspectable runtime contract for model-native tool calls, MCP/tool/skill calls, runtime-local tools, and external device actions; M20 is the next active milestone and establishes the foundation required to resume broader M18
...
- Prefer the next post-M7 milestone sequence to stay narrow and layered: M8 formal terminal edge first, M9 cloud-model agent baseline second, M10 grounding and memory third, M11 terminal/CLI interaction maturity fourth, M12 prompt/context engineering fifth, M13 proposal-formation maturity sixth, M14 model-provider connection reliability and diagnostics seventh, M15 runtime-native credential/runtime-config baseline eighth, M16 post-action deliberation/action loop ninth, M17 multi-edge interaction expansion tenth, M20 Agent Harness and runtime action-loop architecture refactor with unified action/tool governance eleventh and next, paused M18 Agent Harness-controlled observation understanding twelfth after M20, paused M19 bounded-growth/storage-hygiene hardening thirteenth after both M20 and M18, M21 policy learning/review fourteenth, M22 first packaged three-end product slice fifteenth, and M23 Home Assistant / smart-home ecosystem bridge last
...
- This bounded implementation still uses a fixed gate and therefore does not yet realize the final Agent Harness-controlled attention model: it cannot adapt observation relevance from the agent's evolving understanding of the user or current situation, nor consume Edge-defined attention events as the primary sense-first wake signal. Broader M18 is paused until M20 establishes the Harness foundation; M21 may later improve this mechanism from long-running experience rather than define it for the first time.
- M20 reference preparation now includes a local, non-vendored clone of `NousResearch/hermes-agent` at `references/hermes-agent`, recorded at upstream `main` commit `e12626b34fb1024bf00f40f4759647f9cbd3f198`; it is reference material for the required Hermes reuse audit, not yet an OpenHalo dependency or harness integration.
...
- We now pause broader M18 observation understanding and M19 bounded-growth/storage-hygiene hardening until the M20 Agent Harness and runtime action-loop architecture refactor is complete: M20 is the next active milestone, M18 resumes on the resulting Harness foundation, and M19 then hardens the storage behavior of the accepted combined system before M21 policy learning, M22 product packaging, and M23 ecosystem bridging
