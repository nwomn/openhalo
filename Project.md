...
- Diagnostic recording is now starting to move inside module classes rather than being owned by the outer orchestration path: `Local Capability Runtime`, `Proposal Formation`, `Presence Router`, and `Execution Planning` each own their public input/output diagnostic boundaries, while Gateway and Session Link retain only their own boundary records
