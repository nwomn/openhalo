...
- Program releases and durable personal data are separate. The current fixed-commit installer atomically selects `current` but requires an explicit `openhalo stop` and `openhalo start`; it does not restart a live Runtime. `openhalo update --check`, `openhalo update`, and `openhalo rollback` are the intended M22 design: they must verify an immutable release checksum/signature, stage and health-check it, atomically activate it, restart only after success, and restore the preceding release on rollback. Updates must never install directly from mutable `master` or silently reset data; an unsupported persistent-state migration must refuse with an explicit recovery/reset choice.
