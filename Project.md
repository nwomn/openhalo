...
- The M22 connection-direction update is explicit: direct IP-address `ws://` must be a normal owner-facing Runtime/Edge path, not a development exception or a reason to require a domain/reverse proxy. `wss://` remains supported where an owner chooses it. Existing Python and Android endpoint rules still enforce the retired TLS policy, and the Runtime defaults to loopback binding; M22/M20.3 now own removal of the endpoint restriction, a direct address binding/installation path, revised owner documentation, and real Terminal/Android acceptance before this new transport contract can be marked implemented.
