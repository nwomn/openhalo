...
- The M18 architecture direction is now source-neutral interaction pooling: M18 observation admission is only a trigger gate, and every admitted observation-driven turn registers a normal interaction alongside chat, agent-initiative, and action-result-driven turns. Multiple interaction scopes may coexist; only exact causal/idempotency duplicates coalesce, action results return through their interaction/turn correlation, and `Presence Router` arbitrates user-facing delivery across the shared pool.
