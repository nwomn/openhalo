...
- 2026-08-23 persistent Main-session release: `v0.1.24` at commit `cb5ccf2` wires the Runtime-owned Main Hermes identity into ordinary semantic calls and verifies Gateway + SQLite restart recovery. The integration contract reuses the native session when it exists; a missing native session advances the generation and records a recovery audit. Downloaded Release assets passed their SHA-256 check and the installed updater reports this version as available.
