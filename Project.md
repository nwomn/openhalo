...
- M20.3 design baseline is now agreed while implementation remains unstarted: Terminal Edge stays a fully independent Device Edge over the public Edge API, with `Quiet Edge` as its default long-running visual language. Settled visible interactions retain a compact, locally expandable receipt that shows only the authorized public lifecycle, final outcome, timestamps, and real display names of involved devices. The proposed receipt is an optional versioned projection on the existing `interaction_update` delivery surface, never a Terminal trace query or a shortcut into Runtime internals. The detailed design is recorded in `docs/plans/2026-07-29-m20-3-terminal-edge-design.md`.
