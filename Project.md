...
Acceptance criteria for M17.4 Android screen/context observation baseline:
...
- Background delivery survival must be treated as a first-class M17.4 engineering problem rather than a documentation caveat: the phone edge should combine a foreground service, accessibility service, OEM battery-policy guidance, reconnect/backoff, wake/push where available, and local health evidence so the runtime can distinguish real-time observation, temporarily degraded delivery, and unusable mobile observation states
