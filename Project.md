...
- Until a verified persistent-state migration exists, upgrades of that transitional long-running server Runtime use the recorded clean-state reset procedure rather than an unsafe in-place state transformation: stop `openhalo-runtime`, remove `/var/lib/openhalo/runtime-state.json`, sync code while preserving `.venv` and `/etc/openhalo`, restart the service, and verify provider access through the service user's proxy environment. This transitional operator procedure is not the M22 end-user update behavior: the packaged updater must preserve compatible data and refuse unsupported migrations explicitly. Proxy verification must check the selected upstream node against `api.openai.com`, not only the presence of `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY`.
