...
This section is the concise operational baseline. The detailed milestone history
and acceptance evidence below is historical context; when wording conflicts, this
snapshot and the latest linked acceptance record are authoritative.
...
| Accepted baseline | `M17.0` through `M17.7.2`, `M18.1`, `M4.1`, `M20`, `M20.2`, `M20.3`, and `M19` storage hardening |
...
- Goal is a durable strategic baseline; GitHub Project `Initiative` is the active execution theme.
- Existing `M0` through `M23` identifiers are historical and remain unchanged.
- Future milestones use a simple linear identifier; execution hierarchy uses Parent Issues and sub-issues rather than deeper milestone numbering.
- `M17` is the parent Issue for multi-edge expansion; `M17.8`, `M17.9`, and `M17.10` are child Issues.
...
- Adjacent milestone ownership is now summarized by the active route: accepted M20, M20.2, and M20.3 provide the current Harness, progress, and Terminal baselines; M17.8, M17.9, and M17.10 own the next Device Edge expansion; broader M18 owns observation understanding; M21 owns review-gated policy learning; M22 owns productization; and M23 remains the later ecosystem bridge. M19 is accepted and only reopens for regressions.
...
- The first implementation contributes bounded real-world context as normal observations; it must not default to continuous raw camera, microphone, or other raw-environment upload, and uncertain physical signals remain evidence rather than asserted user intent
...
- In progress (M7, M8, M9, M10, M11, M12, M13, M14, M15, M16, M17.0, M17.1, M17.2, M17.3, M17.4, M17.5, M17.6, M17.7.1, M17.7.2, M18.1, the module-boundary diagnostics v1 baseline, M20, M20.2, and M20.3 completed and accepted; M20.2 acceptance combines the complete Runtime/Gateway/Terminal/Console and Android implementation, focused Android automated verification, and user-reported local-development real-device acceptance)
...
- Accepted baseline at the current phase: `M17.0` through `M17.7.2`, `M18.1`, `M4.1`, `M19`, `M20`, `M20.2`, and `M20.3`. The current execution route is `M17.8` -> `M17.9` -> `M17.10` -> broader `M18` -> `M20.1` -> `M21` -> `M22` -> `M23`; detailed historical evidence remains below.
- Current milestone correction: `M20.2` and `M20.3` are accepted, and `M19` is accepted. The current execution route is `M17.8` -> `M17.9` -> `M17.10` -> broader `M18` -> `M20.1` -> `M21` -> `M22` -> `M23`; `M19` reopens only for regressions.
...
- 2026-08-02 raw-media Runtime exploration is explicitly deferred: direct video/audio ingestion remains a future research direction because the current always-on model cost is too high for the first product slice. The active route stays on bounded Edge observations and snapshot grounding; a future Media Session/Media Plane may resume only after an accepted cost/performance budget and benchmark for sampled media, model cascades, or local inference.
...
- M20.3 is accepted complete: Terminal Edge remains a fully independent `Device Edge` over the public API with the `Quiet Edge` long-running visual language, safe locally expandable outcome receipts, public lifecycle progress, paired P-256 authentication, recoverable connection handling, and non-TTY fallback. The isolated configured-provider human acceptance confirmed natural-language interaction, safe progress and receipt rendering, and `/quit` restoration. The full Python regression passes with `669` tests and `4` intentional skips. Android `edge.runtime.v2` WS-first pairing, reconnect, revocation recovery, and endpoint disclosure are accepted on the real phone. The deployed Runtime now runs immutable [`v0.1.4`](https://github.com/nwomn/openhalo/releases/tag/v0.1.4) (`27c50bf`), whose Gateway releases stale closed WebSocket sessions; its post-update real-phone reconnection received and replied to a fresh frame over the existing P-256 identity.

## Next Execution Route

1. `M17.8` mobile sensitive-screen capture governance: close the allowlist-first privacy boundary before expanding what the agent can learn from phone observation.
2. `M17.9` native Windows Desktop Edge baseline: define and implement the dedicated graphical Windows `Device Edge` before M22 packaging; its detailed design remains a separate follow-up discussion.
3. `M17.10` Ambient Home Presence Edge baseline: define and implement one fixed home/desk `Device Edge` for privacy-governed real-world context; later multiple-node household coverage remains a follow-up extension.
4. Broader `M18` Agent Harness-controlled observation understanding: build the edge-led attention and safe-evidence loop on top of the accepted Harness, progress-capable interaction surfaces, and M17.8/M17.9/M17.10 boundaries.
