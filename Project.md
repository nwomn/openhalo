...
- The `v0.1.0` updater's `403 rate limit exceeded` is now traced to its GitHub REST request with `User-Agent: openhalo` travelling through the configured local HTTP proxy: that request reports `0/60` anonymous calls remaining, while the same `openhalo update --check` command with `HTTP_PROXY`, `HTTPS_PROXY`, and `ALL_PROXY` removed immediately resolves the `v0.1.1` candidate. GitHub's canonical static latest-Release manifest URL works through the proxy, and the next Runtime Release is prepared to use that URL plus tag-scoped checksum/archive URLs rather than any REST API discovery call. This preserves the existing HTTPS and manifest/checksum agreement checks. The already-installed v0.1.0 program needs one proxy-bypass update as bootstrap recovery; later normal updates do not depend on the REST quota.
