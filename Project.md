...
- The Runtime pairing foundation is accepted for its backend scope on 2026-07-20, based on user-confirmed backend acceptance: local `personal_runtime.pairing_cli` creates a TTL-bound one-time code, the Gateway atomically exchanges `auth.kind=pairing` for a device-specific `auth.kind=device` credential, and the independent registry persists hashes, device identity/type, use timestamps, and revocation state without raw pairing or device secrets. Gateway and Runtime tests cover one-time consumption, expiry, revocation, persisted reconnect, startup pairing-store selection, legacy shared-token compatibility, and rejection paths that leave a pairing code usable when a frame is invalid or its device already has a live socket. Android end-to-end public-release acceptance remains separate and still requires real-phone `wss://` initial pairing, credential-only reconnect, revocation, and re-pair evidence.
- Android Edge pairing integration has passed the reported real-phone regression: Android no longer falls back to the development shared token when unpaired, and an offline/unauthenticated send no longer persists `Queued mobile.input` as a chat turn before the later successful `Submitted mobile.input` turn. Both development and public Android sessions now require a pairing code or stored device credential; local development may use trusted `ws://` only for pairing transport, never shared-token authentication. Global Chat retains Runtime notification and rendered-reply records, but only a message actually submitted to Runtime becomes a user chat turn; queued input stays diagnostic-only. The Android build now contains no Runtime IP or URL default and no named server cleartext allowlist; existing installed-app configuration remains local user state until manually changed or reset. Focused Kotlin frame tests plus debug and instrumentation APK assembly pass. Full public-release acceptance still requires documented real-phone `wss://` initial pairing, credential-only reconnect, revocation, and re-pair evidence.
