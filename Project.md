...
- The first public-release Gateway hardening slice is now implemented and focused-verified: a real WebSocket must successfully authenticate with `connect` before any post-connect frame is processed, may represent only its bound `device_id`, and cannot silently replace a live socket for the same device. Network regression coverage proves rejection of anonymous event injection, authenticated cross-device spoofing, and duplicate live-device connects; per-device pairing, credential rotation, TLS-only transport, and mobile privacy hardening remain separate release gates.
