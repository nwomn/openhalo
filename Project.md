...
- Gateway enforces registration strictly on the normal path: unregistered observations, schema-mismatched observations, undeclared action-result capabilities, direction mismatches, and device/capability mismatches are rejected with inspectable public errors instead of being silently stored or used
...
- Capability/provider selection consumes proposal requirements, presence governance boundaries, registered capability/provider metadata, validated observations or snapshot fields, and device availability to produce an inspectable candidate list
