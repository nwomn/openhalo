...
- `M17.10` is the owner-authorized active milestone under parent Issue `M17`. Its implemented baseline is a persistent health Camera Edge and bounded local status surface; its active first local-vision slice is opt-in `person_presence.v1`, backed by the installed MaixCAM YOLO11 model and a semantic-only `camera.person_presence` Observation. The next owner-authorized validation step packages it as the manually launched Maix App `openhalo_camera_edge`; the minimal App has been built and installed through the Maix App Store CLI with `main.py` registration and private device configuration, but is not yet Launcher-accepted. It must preserve the public Edge API boundary, use one Edge-owned camera/NPU lifecycle, read Runtime connection settings only from device-private configuration, report `unavailable` rather than a false empty-room result, and must not capture/upload raw media by default. A direct SSH launch produced a vendor multimedia out-of-memory failure and is not a valid substitute for the physical Launcher lifecycle; user-launched App acceptance is required. System-service startup and App boot auto-start remain deferred. The prior `M17.8` mobile sensitive-screen capture governance work remains open and will resume after this bounded camera slice.
