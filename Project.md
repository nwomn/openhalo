...
- In progress (M7, M8, M9, M10, M11, M12, M13, M14, M15, M16, M17.0, M17.1, M17.2, M17.3, M17.4, M17.5, M17.6, M17.7.1, M17.7.2, M18.1, the module-boundary diagnostics v1 baseline, M20, and M20.2 completed and accepted; M20.2 acceptance combines the complete Runtime/Gateway/Terminal/Console and Android implementation, focused Android automated verification, and user-reported local-development real-device acceptance)
...
- M20.3 is in implementation: Terminal Edge stays a fully independent Device Edge over the public Edge API, with `Quiet Edge` as its default long-running visual language. Settled visible interactions retain a compact, locally expandable receipt that shows only the authorized public lifecycle, final outcome, timestamps, and real display names of involved devices. The receipt is an optional versioned projection on the existing `interaction_update` delivery surface, never a Terminal trace query or a shortcut into Runtime internals. The v2 device-key migration now covers Runtime pairing records, Terminal, managed Host, safe receipt projection, and Android's Keystore/client/UI wiring without a bearer fallback. Python focused regression passes for public-key pairing, signed reconnect, legacy-bearer rejection, duplicate-session pairing-code preservation, and Quiet Edge receipts. Android local compilation and real-device pairing remain unverified here because this development environment has no configured Android SDK. The detailed design is recorded in `docs/plans/2026-07-29-m20-3-terminal-edge-design.md`.
- M20.3 Quiet Edge automated slice is now complete: a single bounded chronological Textual transcript renders user/system/runtime output and local compact receipts; public progress is one replaceable row; receipt expansion, composer history, slash completion, and draft cancellation stay local. Drafts persist across recoverable connection loss, and the daemon retries network/WebSocket failures with bounded backoff without creating a Runtime outcome. `openhalo-edge setup` accepts `wss://` endpoints or loopback-only development `ws://`, and owner `openhalo rename` updates the real device name rendered by receipts. The current focused Terminal/CLI/endpoint suite has 91 passing tests, including real v2 pairing and Gateway interaction. The remaining acceptance is a live TTY run and removal or migration of historical bearer-dependent development utilities; Android compilation remains unavailable in this environment.

## Next Execution Route

1. `M20.3` Terminal Edge stable CLI/TUI: turn the accepted lifecycle and existing terminal session into a resilient, application-quality OpenHalo CLI surface.
2. `M17.8` mobile sensitive-screen capture governance: close the allowlist-first privacy boundary before expanding what the agent can learn from phone observation.
3. `M17.9` native Windows Desktop Edge baseline: define and implement the dedicated graphical Windows `Device Edge` before M22 packaging; its detailed design remains a separate follow-up discussion.
4. `M17.10` Ambient Home Presence Edge baseline: define and implement one fixed home/desk `Device Edge` for privacy-governed real-world context; later multiple-node household coverage remains a follow-up extension.
5. Broader `M18` Agent Harness-controlled observation understanding: build the edge-led attention and safe-evidence loop on top of the accepted Harness, progress-capable interaction surfaces, and M17.8/M17.9/M17.10 boundaries.
