...
- M20.3 Android `edge.runtime.v2` WS acceptance is now passed for the source change: Android unit/build verification passed and the rebuilt APK was installed on the real phone; the server Linux pairing regression passed 8/8 including exact same-identity replay and rejection of device, type, audience, and public-key mismatches. A temporary server Runtime overlay at `ws://8.153.37.167:8765` completed fresh P-256 pairing, `connect_ok`, capability announcement, stored-key reconnect, server revocation to `pairing_required`, Android local state clearing to `repair_required=true`, visible `SETUP`/re-pair recovery, and final fresh re-pair back to `device_paired=true`. The phone's final server record is active with the same public-key fingerprint and `revoked_at=null`. The source implementation for the pairing replay/revocation recovery is committed and pushed on `master` as `3c0e590209df03717977ac0ac33431934c106f63`; it is not part of the deployed `v0.1.4` program and still requires a future immutable Release and owner update. The separate Gateway stale-closed-session recovery is published as [`v0.1.4`](https://github.com/nwomn/openhalo/releases/tag/v0.1.4) from `27c50bf0a4d2ccafcf2bd7bf373057dd6c012fd0` and installed through `openhalo update`: on 2026-07-31 the persistent Runtime was verified as `openhalo 0.1.4 (27c50bf)`, listening at `0.0.0.0:8765`; the paired Android Edge then manually reconnected with its existing P-256 identity, Runtime recorded the session as connected, received a fresh phone frame, and dispatched a response without a closed-session send failure. This accepts the deployed closed-session recovery and live WS link; provider/proxy egress remains a separate deferred concern.
