...
- Gateway WebSocket dispatch now emits non-invasive `diagnostic.v1` records for cross-device reply delivery, including target connection presence and send status, and host-edge local action execution now records its own `Local Action Executor` boundary without importing backend internals
