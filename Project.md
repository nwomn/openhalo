...
- 4.21.3. Define milestone M20.3: Terminal Edge stable CLI/TUI, immediately after M20.2, so the resident desktop terminal becomes a durable, ergonomic OpenHalo interaction surface rather than a minimal verifier. It owns an application-quality terminal layout, transcript, composer, interaction/progress rendering, session and connection lifecycle, and non-TTY fallback while continuing to use the normal public Edge API; it is not a thin wrapper around Hermes CLI or a local shell/tool console. The accepted visual direction is `Quiet Edge`: a calm long-running Device Edge session with a concise transcript and compact, expandable outcome receipts. A receipt may show a governed action's safe public lifecycle, final user-facing outcome, timestamps, and the real display names of involved authorized devices, but never Runtime internals, model/provider details, tool arguments/results, private reasoning, or Hermes identity. M20.3 consumes M20.2 progress events when available, but its core stability and layout work can proceed independently. Implementation is now in progress: the first test-driven slice raises the public contract to `edge.runtime.v2`, adds shared P-256 challenge-signature primitives, declares `cryptography` as a product dependency, and migrates persisted pairing records from bearer credentials to revocable device public-key metadata while clearing legacy records. Follow-up implementation has moved Runtime-owned Host startup away from shared-token injection into owner-provisioned local P-256 identity, while keeping its ordinary WebSocket proof connection; it also adds the first safe `outcome_receipt.v1` projection, real registered device-name authorization, and a bounded pure Terminal presentation reducer that keeps compact receipts local and expandable without Runtime inspection access. Android now completes the client/UI migration: its Keystore P-256 identity signs the shared canonical proof for both pairing and reconnect, and no bearer/shared token remains in BuildConfig, preferences, Service Intent, connection configuration, or Compose state. Legacy Android credentials are cleared, require explicit re-pairing, and that state is surfaced in Settings. Runtime/Python focused v2 pairing and Quiet Edge regression coverage passes; Android compile and real-device acceptance remain pending a development environment with a configured Android SDK. Final Terminal TUI interaction, production TLS enforcement, CLI device management, and full regression/acceptance remain open.
- M20.3 Quiet Edge implementation update: the default Textual surface now uses one bounded chronological transcript for user/system/runtime lines and local compact receipts, with one fixed active-progress row and a fixed composer. Receipt expansion remains entirely local; the composer preserves drafts through reconnect, provides `Tab` completion for unique local slash commands, history navigation, and unsent-draft clearing. Terminal reconnect retries recoverable connection failures with bounded backoff, while Python Terminal/Host endpoints reject non-loopback `ws://` and `openhalo-edge setup` cannot persist a plaintext remote endpoint. Owner `openhalo rename <device_id> <display_name>` now completes the real-device-name management loop. Focused reducer/TUI/CLI/endpoint/v2 Gateway regressions pass. Remaining M20.3 acceptance is a live real-TTY run (slow progress, cross-device receipt, reconnect, resize, slash command, clean exit), alongside migration or retirement of historical bearer-based test utilities; M20.3 remains in progress.
