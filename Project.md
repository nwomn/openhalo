...
- Background delivery survival must be treated as a first-class M17.5 engineering problem rather than a documentation caveat: the phone edge should combine a foreground service, accessibility service, OEM battery-policy guidance, reconnect/backoff, wake/push where available, and local health evidence so the runtime can distinguish real-time observation, temporarily degraded delivery, and unusable mobile observation states
- The runtime should participate in mobile observation liveness rather than treating the phone as an isolated client: if a phone edge that recently reported active observation stops delivering `mobile.screen_context` evidence unexpectedly, the runtime should be able to classify the gap, exclude known server/network-side failures where possible, and trigger a bounded mobile wake/recovery path such as FCM or OEM push when configured
- The Android edge should report lifecycle and delivery-risk evidence where the platform allows it, including graceful stop, connection teardown, permission loss, battery restriction changes, and foreground-service health; if the process is killed without callback, the runtime watchdog must treat missing heartbeats/observations as the observable failure signal
- Sensitive contexts are blocked or redacted by default, including password fields, secure windows, configured denylisted apps, and payment/login/banking-like screens where detectable
...
- In progress (`M7`, `M8`, `M9`, `M10`, `M11`, `M12`, `M13`, `M14`, `M15`, `M16`, `M17.0`, `M17.1`, `M17.2`, `M17.3`, `M17.4`, and the module-boundary diagnostics v1 baseline completed and accepted; broader `M17` real multi-edge interaction expansion continues on top of the accepted Android phone edge product UI baseline; `M17.5` is implemented to the automated-verification/debug-APK stage and is ready for real-phone human acceptance; `M17.6` is now defined as the active multi-edge interaction lineage and fail-fast semantics hardening follow-up exposed by terminal-to-phone real-use testing; `M18` observation-driven idle intent sensing and `M19` action/tool governance remain later work; `M20` policy learning/review is intentionally deferred until the system is stable but not fully ergonomic; `M21` first packaged three-end product slice is now the first concrete milestone for productization; storage hardening is deferred to `M22`; Home Assistant Bridge Edge and broader smart-home ecosystem bridging are intentionally deferred to `M23`)
...
- Post-M16 architecture expansion has completed and accepted `M17.0` public Edge API boundary/internal-runtime encapsulation, `M17.1` registration-driven multi-device extension, `M17.2` native Android Presence Edge, `M17.3` Android daily-use hardening, `M17.4` Mobile Edge product UI implementation, and the module-boundary diagnostics v1 baseline. `M17.5` mobile observation depth and `M21` first packaged three-end productization follow after it, while `M18` observation-driven idle intent sensing, `M19` behavior-contract/action-tool governance, and `M20` policy learning/review remain later work and bounded-growth/storage-hygiene hardening remains intentionally deferred to `M22`
