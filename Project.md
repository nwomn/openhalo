...
- 4.17. Define milestone M16: runtime-native credential and auth-store baseline, so OpenHalo can authenticate real model-provider access through its own inspectable login/auth resolution flow instead of depending only on ad hoc shell environment variables or external tool-specific credential stores
...
- In progress (`M7`, `M8`, `M9`, `M10`, `M11`, `M12`, `M13`, and `M14` completed and accepted; active execution focus now moves to `M15` post-action deliberation and interaction action-loop work before `M16` runtime-native credential/auth-store work, `M17` policy learning/review, and `M18` multi-edge interaction expansion, with storage hardening deferred to `M19`)
...
- The repository now includes a tracked default `config/llm-config.toml`, explicit provider-level header configuration for future multi-provider compatibility hardening, and a runtime startup path that only uses non-default LLM config when an explicit config path is provided; the tracked default baseline is currently aligned to the same Cubence-hosted OpenAI-compatible route family that Codex uses on this machine, with the repository default now targeting `https://api-dmit.cubence.com/v1` while keeping the normal runtime auth-env wiring unless an explicit alternate config is requested
...
- We now reorder the post-M7 roadmap so storage hardening is no longer the immediate next step: M8 formal terminal edge, M9 cloud-model-backed agent baseline, M10 model grounding/runtime memory, M11 terminal/CLI interaction maturity, M12 prompt/context engineering, M13 proposal-formation maturity, M14 model-provider connection reliability and diagnostics, M15 post-action deliberation/action loop, M16 runtime-native credential/auth-store baseline, M17 policy learning/review, M18 multi-edge interaction expansion, and M19 bounded-growth/storage-hygiene hardening
...
- We now want runtime-native credential/auth-store work to become the immediate milestone after post-action behavior, so real provider access can become a product-owned capability instead of staying dependent on ad hoc shell environment setup or external tool credential stores
