...
- 4.21. Define milestone M20: Agent Harness and runtime action-loop architecture refactor with unified action/tool governance, so the already-running OpenHalo system can be reshaped around an explicit Hermes-style harness core, nested internal agent loop, external runtime action loop, memory-consolidation model, LLMOps/eval gate, and one inspectable runtime contract for model-native tool calls, MCP/tool/skill calls, runtime-local tools, and external device actions
