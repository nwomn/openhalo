...
- `M17.10` is the owner-authorized active milestone under parent Issue `M17`. Its implemented baseline is a persistent health Camera Edge and bounded local status surface; its active first local-vision slice is opt-in `person_presence.v1`, backed by the installed MaixCAM YOLO11 model and a semantic-only `camera.person_presence` Observation. The minimal Maix App `openhalo_camera_edge` has been built and installed through the Maix App Store CLI with `main.py` registration and private device configuration, and now has narrow physical Launcher lifecycle acceptance: the owner manually launched it and Runtime persisted two fresh `absent` Feature Observations 30 seconds apart, with no raw media upload. It must preserve the public Edge API boundary, use one Edge-owned camera/NPU lifecycle, read Runtime connection settings only from device-private configuration, report `unavailable` rather than a false empty-room result, and must not capture/upload raw media by default. The intentionally UI-less minimal App displays a black screen. A direct SSH launch previously produced a vendor multimedia out-of-memory failure and remains invalid for this App lifecycle. A `present` scene test, single-process local status UI, system-service startup, and App boot auto-start are separate future decisions. The prior `M17.8` mobile sensitive-screen capture governance work remains open and will resume after this bounded camera slice.
