...
5. Broader `M18` Agent Harness-controlled observation understanding: build the edge-led attention and safe-evidence loop on top of the accepted Harness, progress-capable interaction surfaces, and M17.8 boundary.
6. `M19` bounded growth and storage hygiene: set retention, compaction, and operational pressure controls after the final observation flow is known.
7. `M20.1` governed procedural-memory and skill lifecycle: introduce inspectable, OpenHalo-owned skill drafts only after the relevant evidence, retention, and audit shape has stabilized.
8. `M21` policy learning and review: turn feedback and replay evidence into review-gated policy candidates.
9. `M22` productization: package the server/runtime, desktop, and phone as one installable three-end system.
10. `M23` Home Assistant and smart-home bridge: add the ecosystem bridge after the core product surface is stable.
