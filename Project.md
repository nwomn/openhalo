...
- The M17 Android edge acceptance workflow is now documented in `docs/m17-android-edge-acceptance.md` as a three-layer practice: runtime-side simulated verification for routing and lineage, adb-based Android real-device smoke verification for app/device behavior, and manual live-chain acceptance for the full `terminal edge -> runtime -> Android phone edge` path. Normal development should run the first layer by default, Android app changes should also run the second layer, and full cross-device live validation remains a milestone/human acceptance step rather than a mandatory check for every edit.
