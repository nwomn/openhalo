...
- Any proposed improvement keeps the architectural boundary intact: model-backed proposal formation remains responsible for deciding continue/complete, but prompt contracts, validation, repair, provider retry, and failure containment may be strengthened around it
...
- Human acceptance demonstrates a real phone observation session where foreground-app observations arrive near real time, the phone edge is interrupted or background-restricted, the runtime detects the gap, triggers the configured wake/recovery path, and either restores fresh observation delivery or marks the phone observation surface degraded/unavailable with inspectable evidence
...
- Any proposal produced from idle observation sensing still uses the normal proposal taxonomy (`reply`, `action`, `clarification`, `no_intervention`) and must pass through `Presence Router` before surfacing to a user or dispatching an action
- The design distinguishes idle/standby observation intent sensing from M16 active-interaction re-entry: M16 handles new evidence inside an existing interaction lifecycle, while M18 starts a new candidate interaction from the broader observation context
...
Acceptance criteria for M19 agent behavior contracts and unified action/tool governance:
...
- Human acceptance demonstrates a real or simulated terminal/host/multi-edge scenario where an action result re-enters model-backed deliberation, produces a non-fixed follow-up proposal, passes validation and `Presence Router`, and records complete action/tool lineage

Acceptance criteria for M20 policy learning and review loop:
...
- In progress (`M7`, `M8`, `M9`, `M10`, `M11`, `M12`, `M13`, `M14`, `M15`, `M16`, `M17.0`, `M17.1`, `M17.2`, `M17.3`, `M17.4`, `M17.5`, and the module-boundary diagnostics v1 baseline completed and accepted; broader `M17` real multi-edge interaction expansion continues on top of the accepted Android phone edge product UI and screen-observation baselines; `M17.6` is now defined as the active multi-edge interaction lineage and fail-fast semantics hardening follow-up exposed by terminal-to-phone real-use testing; `M17.7` is now defined as the dedicated mobile observation liveness and wake-recovery milestone split out from the Android screen/context observation baseline; `M17.8` is now defined as the dedicated mobile sensitive-screen capture governance milestone split out from M17.5 after live banking-screen testing showed best-effort detection is not a sufficient privacy boundary; `M18` observation-driven idle intent sensing and `M19` action/tool governance remain later work; `M20` policy learning/review is intentionally deferred until the system is stable but not fully ergonomic; `M21` first packaged three-end product slice is now the first concrete milestone for productization; storage hardening is deferred to `M22`; Home Assistant Bridge Edge and broader smart-home ecosystem bridging are intentionally deferred to `M23`)
...
- The first M17.6 runtime-semantics hardening slice is being corrected after live acceptance exposed an invalid deterministic action-loop shortcut: repeated terminal output showed that source acknowledgement and loop stopping cannot be fixed by local `prior_proposal` guards or deterministic fallback wording. The current M17.6 direction is that every `action_result` re-enters `Agent Runtime / Proposal Formation` with frame lineage (`device_id`, `request_id`, `interaction_id`), prior proposal, interaction source/target/participants, and result evidence; the proposal itself must explicitly continue or complete the loop, while deterministic fallback must not decide cross-surface acknowledgement or normal loop completion semantics. Automated coverage now includes an injected proposal-formation harness for re-entry/stop behavior, preserves model `no_intervention` without local rewrite, and keeps unknown action-result lineage returning a structured public `lineage_missing` error. M17.6 remains open until a real terminal-to-phone manual live-chain run with model-backed proposal formation confirms that the phone receives the notification and the terminal receives the acknowledgement or failure explanation through the normal runtime chain without repeated fixed runtime messages.
...
- Post-M16 architecture expansion has completed and accepted `M17.0` public Edge API boundary/internal-runtime encapsulation, `M17.1` registration-driven multi-device extension, `M17.2` native Android Presence Edge, `M17.3` Android daily-use hardening, `M17.4` Mobile Edge product UI implementation, `M17.5` Android screen/context observation baseline, and the module-boundary diagnostics v1 baseline. `M17.6` multi-edge lineage/fail-fast semantics, `M17.7` mobile observation liveness and wake recovery, `M17.8` mobile sensitive-screen capture governance, and `M21` first packaged three-end productization follow after it, while `M18` observation-driven idle intent sensing, `M19` behavior-contract/action-tool governance, and `M20` policy learning/review remain later work and bounded-growth/storage-hygiene hardening remains intentionally deferred to `M22`
...
- We now want multi-edge interaction expansion to become the immediate milestone after post-action behavior, because the system needs more real device surfaces before deeper idle sensing, action/tool governance, or policy learning can be validated against representative behavior
- We now intentionally defer policy learning and review to M20, after multi-edge expansion, idle observation-driven intent sensing, and action/tool governance make the system stable but not yet fully ergonomic enough for model-backed feedback interpretation to improve real usage rather than only synthetic scenarios
...
- The current M17.6 hardening slice now re-enters proposal formation after every recorded action result, preserves action-result frame lineage, keeps action-request ids runtime-owned, and avoids local deterministic source-ack shortcuts; follow-up provider hardening now gives transient protocol-shape failures such as repeated `codex_agent_envelope_empty_output` one additional provider-layer retry before surfacing a user-visible provider error. Manual terminal-to-phone acceptance still needs to be rerun before M17.6 can be accepted.
- The latest M17.6 manual terminal-to-phone acceptance still fails the full criterion: one live run delivered `notification.show` to the Android phone and then re-entered model-backed post-action proposal formation, but the terminal source edge received no acknowledgement before the model chose `no_intervention`, so only the target action surface succeeded. A later Chinese command also exhausted provider shape retries and routed the provider error as user-visible notification content, confirming that M17.6 still needs source-facing acknowledgement/failure semantics and provider-failure containment before acceptance.
- M17.6.1 is now split out as the first Agent Runtime harness engineering baseline, starting from the low success rate of model-backed proposal formation under real M17.6 prompt/context packages. The goal is to capture, classify, replay, and improve proposal formation before treating source acknowledgement as a narrow prompt tweak, because the observed failures include both protocol-shape instability and semantically incomplete proposals; the resulting corpus/harness pattern should become the starting template for later Agent Runtime behavior hardening.
- Observation-driven intent sensing is now explicitly split into a later M18 milestone after multi-edge expansion: M16 continues to own re-entry inside an active interaction lifecycle, while M18 will own the idle/standby behavior where the runtime periodically evaluates the full current observation context across edge devices and decides whether to form a new proactive proposal
