...
- The follow-up M22.1 proxy-test regression fix is prepared for `v0.1.7`: the Runtime probe now passes its keyword-only proxy and timeout arguments correctly, and a regression test covers the default `openhalo proxy test` provider probe. The intended live Mihomo endpoint is `http://127.0.0.1:7890`; installed Runtime acceptance and Release publication remain in progress for this turn.
