...
- Program releases and durable personal data are separate. `openhalo update --check` checks an immutable published release, `openhalo update` verifies its checksum/signature, stages it, runs a bounded health check, atomically selects it as `current`, and restarts the hidden per-user Runtime only after success. `openhalo rollback` restores the preceding release. Updates must never install directly from mutable `master` or silently reset data; an unsupported persistent-state migration must refuse with an explicit recovery/reset choice.
...
- `openhalo --version`, `openhalo update --check`, `openhalo update`, and `openhalo rollback` are implemented and manually verified. Update verification requires an immutable release reference plus checksum/signature verification, staging, a bounded health check, atomic activation, and automatic preservation of the prior active release on failure; rollback restores the previous program release without silently reverting or erasing personal data
