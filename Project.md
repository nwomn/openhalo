...
- Gateway binds each real WebSocket to exactly one successfully authenticated `device_id`: post-connect frames must carry that same identity, unauthenticated frames receive `not_connected`, cross-device frames receive `device_mismatch`, and a second live socket cannot silently replace an existing device session. Runtime now provides a generic local pairing registry: an administrator-created one-time code yields a device-specific credential, persists only hashes and safe device metadata, supports revocation, and survives Runtime restart. The first Edge must persist its own Runtime URL and returned credential; Android UI/storage integration, full Host/Terminal migration away from legacy shared-token compatibility, and release-only `wss://` enforcement remain separate public-release work.
...
- 4.21.3. Define milestone M20.3: Terminal Edge stable CLI/TUI, immediately after M20.2, so the resident desktop terminal becomes a durable, ergonomic OpenHalo interaction surface rather than a minimal verifier. It owns an application-quality terminal layout, transcript, composer, interaction/progress rendering, session and connection lifecycle, and non-TTY fallback while continuing to use the normal public Edge API; it is not a thin wrapper around Hermes CLI or a local shell/tool console. M20.3 consumes M20.2 progress events when available, but its core stability and layout work can proceed independently. The first Terminal-only implementation slice is now complete: the Textual surface has a compact connection header, structured role-labelled transcript, an in-place Active Interaction panel, fixed composer, slash discovery/completion, in-memory input history, visible-transcript clearing, responsive narrow/short layout, cancellable queue input, bounded reconnect with draft preservation, and unchanged line-mode/`TERM=dumb` fallback. Focused automated coverage passes 120 tests with 1 intentional skip; a macOS foreground run completed `hello runtime`, then preserved the TUI process across a Runtime stop and automatically restored the WebSocket session after restart. This is implementation and local acceptance for the first slice, not acceptance of the full M20.3 milestone: a longer configured-provider session with slow model work and a healthy cross-device action/result path remains pending, and the macOS Host Edge currently cannot complete `runtime.status` because its existing process adapter expects `/proc`.
...
- M20.3 has completed its first Terminal-only implementation and local-acceptance slice without changing the public Edge API: the Textual UI now separates role-labelled conversation from an in-place Active Interaction panel, adds connection state, responsive layout, slash discovery/completion, in-memory input history, draft-preserving reconnect, clean queue cancellation, and unchanged line/`TERM=dumb` fallback. The focused Terminal/progress/CLI/docs/boundary suite passes 120 tests with 1 intentional skip. A real macOS Terminal session completed a deterministic `hello runtime` interaction and automatically reconnected after the Runtime was stopped and restarted. Full M20.3 acceptance remains open for a longer configured-provider session, slow-turn coverage, and a healthy cross-device action/result run; the attempted local `runtime.status` run correctly reached `awaiting_action_result`, but the existing Host Edge process adapter failed on macOS because it reads `/proc`.
