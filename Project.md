...
- `M17.10` is the owner-authorized active milestone under parent Issue `M17`. Its implemented baseline is a persistent health Camera Edge and bounded local status surface; its active first local-vision slice is opt-in `person_presence.v1`, backed by the installed MaixCAM YOLO11 model and a semantic-only `camera.person_presence` Observation. The minimal Maix App `openhalo_camera_edge` has been built and installed through the Maix App Store CLI with `main.py` registration and private device configuration, and now has narrow physical Launcher lifecycle acceptance: the owner manually launched it and Runtime persisted fresh `absent` Observations 30 seconds apart, then persisted repeated `present` Observations while one person stood in view (latest `{state: "present", count: 1}` at `2026-08-23T10:22:24.248688Z`, confidence `0.859375`), with no raw media upload. The first `0.1.1` icon package retained the Launcher default icon despite valid metadata; package `0.1.2` replaced it with the same confirmed OpenHalo primary logo as a Launcher-compatible `128×128` RGBA PNG with world-readable mode, and the owner visually confirmed the corrected Launcher icon after restart. This is a Launcher-presentation change only. Camera observations reach Runtime's proactive admission gate but currently record `not_high_salience` and do not invoke Agent proposal formation; promoting them into a compact snapshot or proactive policy is a separate governance decision. This accepts the basic person-presence state transition only; it is not identity or multi-person accuracy evaluation. It must preserve the public Edge API boundary, use one Edge-owned camera/NPU lifecycle, read Runtime connection settings only from device-private configuration, report `unavailable` rather than a false empty-room result, and must not capture/upload raw media by default. The intentionally UI-less minimal App displays a black screen. A direct SSH launch previously produced a vendor multimedia out-of-memory failure and remains invalid for this App lifecycle. A single-process local status UI, system-service startup, and App boot auto-start are separate future decisions. The prior `M17.8` mobile sensitive-screen capture governance work remains open and will resume after this bounded camera slice.
