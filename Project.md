...
- The Runtime pairing foundation is implemented and ready for Edge integration: local `personal_runtime.pairing_cli` creates a TTL-bound one-time code, the Gateway atomically exchanges `auth.kind=pairing` for a device-specific `auth.kind=device` credential, and the independent registry persists hashes, device identity/type, use timestamps, and revocation state without raw pairing or device secrets. Gateway and Runtime tests cover one-time consumption, expiry, revocation, persisted reconnect, startup pairing-store selection, legacy shared-token compatibility, and rejection paths that leave a pairing code usable when a frame is invalid or its device already has a live socket. This is not yet end-to-end accepted: the next step is an Edge that persists the Runtime URL plus returned device credential and completes a real `wss://` pairing/reconnect/revocation scenario.
