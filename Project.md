...
- Event-triggered upload is primary and periodic upload is only a fallback: UI events are debounced/coalesced, typing is not uploaded per character, unchanged screens avoid repeated full payloads, and screen-off/locked states pause rich extraction by default
...
- Runtime ingestion treats `mobile.screen_context` as passive context evidence for later snapshot/grounding, not as a direct command or as M18 observation-driven intent sensing
...
- Human acceptance demonstrates that a real phone can enable the feature, interact with several normal apps, and produce useful bounded screen-context observations without raw screenshot upload or runtime intervention
