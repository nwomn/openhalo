...
- 4.16. Define milestone M15: post-action deliberation and interaction action loop, so action results or fresh observations can re-enter `Agent Runtime` inside the same interaction lifecycle and yield a new inspectable `reply`, `action`, `clarification`, or `no_intervention` outcome instead of terminating at a fixed completion formatter
- 4.17. Define milestone M16: runtime-native credential and runtime-config baseline, so OpenHalo can authenticate real model-provider access through its own inspectable local runtime configuration flow instead of depending on ad hoc shell environment variables or external tool-specific credential stores
...
- 4.20. Define milestone M19: bounded-growth and storage-hygiene hardening pass after the first mature product slice, covering unbounded state growth, high-frequency persistence pressure, duplicated long-term storage, and other operational accumulation risks across the system
...
- Adjacent milestones deepen that behavior from other angles without changing the ownership boundary: `M9` supplies provider-backed generation, `M10` supplies runtime grounding and memory, `M12` supplies prompt/context and behavior-contract hardening, `M14` supplies model-provider reliability and diagnostics, `M15` supplies post-action deliberation and same-interaction action-loop re-entry, and later milestones deepen operator trust and adaptation through runtime-native credentials (`M16`) and review-gated policy learning (`M17`) once both proposal typing and post-action behavior are mature enough to evaluate. The current M16 implementation begins with a narrow single-file local runtime-config baseline rather than an environment-variable fallback.
...
Acceptance criteria for M15 post-action deliberation and interaction action loop:
...
Acceptance criteria for M16 runtime-native credential and runtime-config baseline:
...
- Temporary completion-formatting patches should not be treated as substitutes for `M15`: interaction completion may carry structural status and edge delivery updates, but semantic post-action deliberation belongs to the explicit action-loop milestone
...
- In progress (`M7`, `M8`, `M9`, `M10`, `M11`, `M12`, `M13`, and `M14` completed and accepted; M16 runtime-native credential/runtime-config work has started with a first single-file local config slice; M15 post-action deliberation/action-loop work remains pending acceptance; `M17` policy learning/review and `M18` multi-edge interaction expansion remain later work, with storage hardening deferred to `M19`)
...
- The accepted `M13` boundary stops at first-turn proposal typing plus primary-action dispatch; post-action semantic handling remains intentionally out of scope here and is now promoted into explicit `M15` action-loop work rather than being represented by a completion-summary patch
...
- Post-M14 architecture expansion is now moving into `M15` post-action deliberation and interaction action-loop work; `M14` model-provider connection reliability and diagnostics is accepted as complete, while `M16` runtime-native credential/runtime-config work, `M17` policy learning/review, and broader `M18` multi-edge interaction expansion remain later work and bounded-growth/storage-hygiene hardening remains intentionally deferred to `M19`
...
- Interaction completion currently remains a structural live-chain surface only: it can mark an interaction finished and notify user-facing edges about delivery or remote action completion, but semantic post-action deliberation is now explicitly deferred to `M15` rather than hidden behind a formatter patch
...
- We now reorder the post-M7 roadmap so storage hardening is no longer the immediate next step: M8 formal terminal edge, M9 cloud-model-backed agent baseline, M10 model grounding/runtime memory, M11 terminal/CLI interaction maturity, M12 prompt/context engineering, M13 proposal-formation maturity, M14 model-provider connection reliability and diagnostics, M15 post-action deliberation/action loop, M16 runtime-native credential/runtime-config baseline, M17 policy learning/review, M18 multi-edge interaction expansion, and M19 bounded-growth/storage-hygiene hardening
- We now intentionally defer bounded-growth and storage-hygiene hardening to M19, after the first formal terminal/model interaction surfaces plus explicit proposal-formation maturity, model-provider reliability, action-loop, credential-baseline, and policy-learning design have been validated more concretely
...
- We now want post-action deliberation and same-interaction action-loop handling to remain its own explicit milestone after model-provider reliability, rather than being approximated by a completion-summary patch on the gateway path
- We now want runtime-native credential/runtime-config work to become the immediate milestone after post-action behavior, so real provider access can become a product-owned capability instead of staying dependent on ad hoc shell environment setup or external tool credential stores
- We now want policy learning and review to remain its own later milestone after proposal formation, post-action behavior, and runtime-native credentialing all mature, rather than being hidden inside the first provider-integration batch
...
- Runtime config precedence is now intentionally local and single-source: ordinary startup uses ignored `config/runtime-config.toml` unless an operator explicitly passes `--llm-config-path`, while `config/runtime-config.example.toml` remains the tracked template for the expected provider/model/profile/API-key shape
...
- M14 is now accepted complete; any further provider dashboards, broader live-provider matrices, or deeper operational telemetry are follow-up hardening rather than blockers for `M15` action-loop semantics
