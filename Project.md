...
- The supervised server Runtime on `8765` has been synchronized from the obsolete `/opt/openhalo` snapshot to the current Runtime source and dependency set. The update followed the required clean-state path by removing the old `/var/lib/openhalo/runtime-state.json` while preserving `/etc/openhalo` configuration and the service virtual environment; the restarted service is loopback-only, its managed `host-edge-1` registered through the normal Gateway path, and its freshly persisted state remains owner-only (`0600`) after a real save. The state-store permission fix uses an owner-only atomic temporary file rather than relying on system umask. Any previously registered server Runtime devices must reconnect or pair again after this state reset.
