...
- `M17.11` and `M17.9` remain queued Device Edge child Issues in the active route before the full M17.10 milestone. In parallel, M17.10 has a bounded in-progress physical Camera Edge Feature-validation slice using the received Sipeed MaixCAM standard kit with a bounded read-only diagnostic display; this does not advance M17.10 ahead of the route or accept it. M17.10 remains unaccepted until its separately documented Observation, governance, evidence, privacy, and human-acceptance criteria are met.
