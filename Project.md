...
- 4.21. Define milestone M20: Agent Harness and runtime action-loop architecture refactor with unified action/tool governance, so the already-running OpenHalo system replaces its real-use agent-execution implementation with a Hermes-backed harness core behind an OpenHalo adapter, while preserving the nested internal agent loop, external runtime action loop, Hermes-native persistent memory, LLMOps/eval gate, and one inspectable runtime contract for model-native tool calls, MCP/tool/skill calls, runtime-local tools, and external device actions; M20 is accepted after its curated Hermes read-only research, durable-memory capabilities, and canonical notification-action contract pass configured-provider human acceptance
- 4.21.1. Define milestone M20.1: governed procedural-memory and skill lifecycle, immediately after M20, so Hermes can distill proven workflows into OpenHalo-owned skill drafts without enabling Hermes' generic skill/plugin execution surface; drafts must have provenance, bounded declarative scope, inspection, static validation, and an explicit activation path before they can affect a live harness turn
...
- In progress (M7, M8, M9, M10, M11, M12, M13, M14, M15, M16, M17.0, M17.1, M17.2, M17.3, M17.4, M17.5, M17.6, M17.7.1, M17.7.2, M18.1, and the module-boundary diagnostics v1 baseline completed and accepted; the M20 Harness foundation, fresh Terminal Edge human revalidation, Android canonical `notification.show` payload migration, and Android local human acceptance are complete and accepted; broader M17 real multi-edge interaction expansion continues; M17.8 remains the dedicated mobile sensitive-screen capture governance milestone; broader M18 observation understanding may build on the Harness foundation, while M19 bounded-growth/storage-hygiene hardening remains paused until broader M18 is accepted; M20.1 governed skill drafts, M21 policy learning/review, M22 productization, and M23 ecosystem bridging remain deferred)
...
- Prefer the post-M7 milestone sequence to stay narrow and layered: M8 formal terminal edge first, M9 cloud-model agent baseline second, M10 grounding and memory third, M11 terminal/CLI interaction maturity fourth, M12 prompt/context engineering fifth, M13 proposal-formation maturity sixth, M14 model-provider connection reliability and diagnostics seventh, M15 runtime-native credential/runtime-config baseline eighth, M16 post-action deliberation/action loop ninth, M17 multi-edge interaction expansion tenth, accepted M20 Agent Harness and runtime action-loop refactor eleventh, broader M18 Agent Harness-controlled observation understanding twelfth and now active on that foundation, paused M19 bounded-growth/storage-hygiene hardening thirteenth after broader M18, M21 policy learning/review fourteenth, M22 first packaged three-end product slice fifteenth, and M23 Home Assistant / smart-home ecosystem bridge last
...
- We completed the M20 Harness foundation after a configured-provider Gateway/terminal human-acceptance ladder proved the governed outer action loop, real public fetch/search, research-assisted reply, hostile fixture provenance with no action or memory side effect, and fresh-session Hermes native-memory recall with body-free evidence. A later natural terminal greeting exposed the legacy `{message}` mismatch; the canonical `notification.show {title?, body}` repair has now passed fresh Terminal Edge human acceptance, including the OpenHalo identity check. Android migration and local human acceptance are now the only remaining M20 acceptance step. M20.1 remains the owner of governed procedural-memory and skill lifecycle work, and M19 follows broader M18 before M21 policy learning, M22 product packaging, and M23 ecosystem bridging
- Follow-up M20 Android notification-contract migration is now implemented: the Android Edge announces and accepts only `notification.show {title?, body}`, returns title/body in its action result, rejects the legacy message-only payload, and uses the ordinary message-notification presentation path. Focused Android unit tests pass (11 tests), and both debug and instrumentation APKs build successfully. No Android device or emulator is attached in this session, so the remaining M20 acceptance step is local human verification of the real Gateway-to-phone notification/action-result path.
- User-confirmed Android local human acceptance has now exercised the migrated notification contract on the current backend path. With the Gateway-to-phone notification and correlated action-result path verified, M20 is accepted complete; broader M18 may proceed on the accepted Harness foundation.
