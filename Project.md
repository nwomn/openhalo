...
- `M17.11` is the next Device Edge child Issue after the active `M17.8`; `M17.9` and `M17.10` follow it as the native Windows and ambient-context child Issues under `M17`. `M17.10` hardware validation has a received Sipeed MaixCAM standard kit with a bounded read-only diagnostic display; stock-device bring-up is in progress. It remains a planning/preparation track and does not activate the milestone before `M17.8`, `M17.11`, and `M17.9`.
