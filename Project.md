...
- `M18` / Issue `#17` is now owner-authorized as the active Runtime refactor. Its first implementation slice introduces independent ContextFact, ContextEnvelope, Main-session, Scheduler, and attention-validation modules with unit-level contracts; Gateway remains only ingress validation, Observation persistence, ContextFact materialization, and handoff. The Main-session manager now creates the native Hermes session before persisting its Runtime-owned identity, injects the bound native `SessionDB` and restored history into normal Hermes semantic calls, restores that session after SQLite-backed Runtime restart when available, and otherwise creates a new audited generation. The remaining M17 Edge milestones retain their scope and provide later physical acceptance inputs.
