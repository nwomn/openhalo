...
- 真实本地 `Personal Runtime` 和 WebSocket Edge API 链路
- host-class edge 和常驻 terminal edge
- 第一版 Android 手机 Edge 表面：`Connect`、`Global Chat`、`Settings`
- 公开 Edge API 的注册、观察、事件、动作和动作结果帧
- 带 prompt/context 检查和 provider diagnostics 的模型驱动 proposal formation
- 经过 Presence 治理的多边缘 action 路由

实现进度按产品切片维护：

| 方向 | 当前状态 |
| --- | --- |
| Runtime core | 已有实现基线，包含 Gateway、state/context、proposal formation、Presence Router、action dispatch、grounding 和 diagnostics |
| Terminal edge | 已完成第一版常驻 terminal edge，支持前台用户输入和 runtime-delivered messages |
| Host edge | 已实现 host-class edge，用于 runtime/host-device 验证和本地动作 |
| Android phone edge | 已完成第一版产品 UI：`Connect`、`Global Chat`、`Settings`、隐藏诊断入口和 preview APK |
| Mobile observation depth | 下一个 Android 里程碑 `M17.5`，重点是屏幕/使用上下文 evidence，而不是在手机侧做 intent decision |
| Product packaging | 后续 `M21`，覆盖可安装三端交付和更完整的 release packaging |
...
- **开发循环**：本地开发 runtime 使用 `18765` 端口，加上本地 host/terminal edges，以及模拟器或手机 edge。
