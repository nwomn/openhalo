...
def main():
    ...
    p.add_argument('--appearance',choices=['pixel','histogram'],default='pixel')
    ...
    threshold=.12 if a.appearance=='pixel' else .35
    s=Scheduler();report=dict(status='loading',config=asdict(s.config),appearance_mode=a.appearance,appearance_threshold=threshold,
    ...
    try:
        ...
        for obs in old['samples']:
            ...
            if hands:
                ...
                if a.appearance=='pixel':
                    signature=cv2.resize(cv2.GaussianBlur(roi,(5,5),0),(16,16)).astype(np.float32)/255
                else:
                    hsv=cv2.cvtColor(roi,cv2.COLOR_BGR2HSV)
                    signature=cv2.calcHist([hsv],[0,1,2],None,[8,4,4],[0,180,0,256,0,256])
                ...
                if last_box is None or now-last_hand>.65 or iou(last_box,chosen['bbox'])<.1:
                    ...
                    dist=float(np.mean(np.abs(signature-anchor))) if a.appearance=='pixel' else float(cv2.compareHist(signature,anchor,cv2.HISTCMP_BHATTACHARYYA))
                    if dist>=threshold:revision+=1;anchor=signature
